---
title: "PGP"
description: "Импортируйте наши ключи GPG, чтобы проверять выпуски и подписанное содержимое."
hide:
  - navigation
schema:
  "@context": https://schema.org
  "@type": Organization
  "@id": https://anonymousplanet.net/
  name: Anonymous Planet PGP
  url: https://anonymousplanet.net/pgp/
  logo: ../../media/profile.png
  sameAs:
    - https://github.com/Anon-Planet
    - https://opencollective.com/anonymousplanetorg
---

<div class="hero-block">
  <div class="hero-eyebrow">Доверяйте, но проверяйте.</div>
  <h1 class="hero-title">Наши открытые ключи<span class="hero-subtitle">Импортируйте. Проверяйте. Ничему не доверяйте вслепую.</span></h1>
  <p class="hero-tagline">
    Anonymous Planet подписывает все выпуски и сообщения ключами GnuPG (PGP).
    Перед импортом сверяйте отпечатки с несколькими независимыми источниками.
  </p>
  <div class="hero-cta-row">
    <a href="anonymousplanet.asc" class="hero-cta hero-cta--primary">Скачать открытую связку ключей</a>
    <a href="#keys" class="hero-cta hero-cta--secondary">Посмотреть отпечатки</a>
  </div>
</div>

---

## Наши ключи { #keys }

Полная связка ключей находится по адресу `https://anonymousplanet.net/pgp/anonymousplanet.asc`.<br>Перед импортом всегда сверяйте отпечатки с нашими [выпусками GitHub](https://github.com/Anon-Planet/thgtoa/releases).

<div class="index-grid">

  <div class="index-card">
    <h3 class="index-card__title">Главный ключ подписи (MSK)</h3>
    <p class="index-card__body">Объявления, подключи подписи и якорь доверия. Корень нашей сети доверия.</p>
    <code class="pgp-fingerprint">9FA5 436D 0EE3 6098 5157 3825 17EC A05F 768D EDF6</code>
  </div>

  <div class="index-card">
    <h3 class="index-card__title">Ключ подписи выпусков (RSK)</h3>
    <p class="index-card__body">Подписывает все выпуски руководства и артефакты распространения. Сверяйте его с каждым выпуском.</p>
    <code class="pgp-fingerprint">C302 3DBE A3FB 38C4 38BA 1EED CEC6 0AED E8B9 92A2</code>
  </div>

  <div class="index-card">
    <h3 class="index-card__title">Ключ шифрования / подписи электронной почты (ESK)</h3>
    <p class="index-card__body">Используйте его, чтобы отправить нам зашифрованное письмо или проверить подписанную нами корреспонденцию.</p>
    <code class="pgp-fingerprint">FCBD 2CAB DEFD 1FBA 2E9E 7591 A1A8 2CD2 DD2C F890</code>
  </div>

</div>

---

## Ротация ключей { #rotation }

Ключи могут периодически меняться. О ротации всегда объявляется через [GitHub Releases](https://github.com/Anon-Planet/thgtoa/releases) и [журнал изменений](../changelog/index.md). Если сохранённый у вас ключ не указан здесь, считайте его скомпрометированным и импортируйте ключ заново.

---

## Открытая связка ключей { #keyring }

<div class="donate-address-block">
  <div class="donate-address-label">URL связки ключей</div>
  <code class="donate-address">https://anonymousplanet.net/pgp/anonymousplanet.asc</code>
</div>

```txt
-----BEGIN PGP PUBLIC KEY BLOCK-----


=ww/S
-----END PGP PUBLIC KEY BLOCK-----
```
